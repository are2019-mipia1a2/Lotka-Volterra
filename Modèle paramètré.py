...
def cgmt(*args) :
    for e4 in list(dict_esp4m) :
            ...
            if dict_esp4m[e4][1]<=faim_e4 :        #Si l'animal a faim
              ...
              if len(proche(e4[0],e4[1],3,-3,dict_esp3m, dict_esp3f,per_4,Terrain_final))==3 and len(proche(e4[0],e4[1],2,-2,dict_esp2m, dict_esp2f,per_4,Terrain_final))==3 :
                    e2,_,de2=proche(e4[0],e4[1],2,-2,dict_esp2m, dict_esp2f,per_4,Terrain_final)
                    e3,_,de3=proche(e4[0],e4[1],3,-3,dict_esp3m, dict_esp3f,per_4,Terrain_final)
                    if len(proche(e4[0],e4[1],e2,e3,de2,de3,1,Terrain_final))==3:
                        cible,poscible,dict_cible=proche(e4[0],e4[1],e2,e3,de2,de3,1,Terrain_final)
                        predation(dict_esp4m,dict_cible,cible,e4[0],e4[1],poscible[0],poscible[1],Terrain_final)
                    else:
                        cible,poscible,dict_cible=proche(e4[0],e4[1],e2,e3,de2,de3,per_4,Terrain_final)
                        deplacement(e4[0],e4[1],per_4,cible,Terrain_final,dict_esp4m,4)
              elif len(proche(e4[0],e4[1],3,-3,dict_esp3m, dict_esp3f,per_4,Terrain_final))==3:
                  cible,poscible,dict_cible=proche(e4[0],e4[1],3,-3,dict_esp3m,dict_esp3f,per_4,Terrain_final)
                  if len(proche(e4[0],e4[1],3,-3,dict_esp3m, dict_esp3f,1,Terrain_final))==3:
                      predation(dict_esp4m,dict_cible,cible,e4[0],e4[1],poscible[0],poscible[1],Terrain_final)
                  else:
                      deplacement(e4[0],e4[1],per_4,cible,Terrain_final,dict_esp4m,4)
              elif len(proche(e4[0],e4[1],2,-2,dict_esp2m, dict_esp2f,per_4,Terrain_final))==3 :
                  cible,poscible,dict_cible=proche(e4[0],e4[1],2,-2,dict_esp2m,dict_esp2f,per_4,Terrain_final)
                  if len(proche(e4[0],e4[1],2,-2,dict_esp2m, dict_esp2f,1,Terrain_final))==3:
                      predation(dict_esp4m,dict_cible,cible,e4[0],e4[1],poscible[0],poscible[1],Terrain_final)
                  else:
                      deplacement(e4[0],e4[1],per_4,cible,Terrain_final,dict_esp4m,4)
    ...
    for e4 in list(dict_esp4f) :
            ...
            if dict_esp4f[e4][1]<=faim_e4 :        #Si l'animal a faim
              ...
              if len(proche(e4[0],e4[1],3,-3,dict_esp3m, dict_esp3f,per_4,Terrain_final))==3 and len(proche(e4[0],e4[1],2,-2,dict_esp2m, dict_esp2f,per_4,Terrain_final))==3 :
                    ...
                    if len(proche(e4[0],e4[1],e2,e3,de2,de3,1,Terrain_final))==3:
                        cible,poscible,dict_cible=proche(e4[0],e4[1],e2,e3,de2,de3,1,Terrain_final)
                        predation(dict_esp4f,dict_cible,cible,e4[0],e4[1],poscible[0],poscible[1],Terrain_final)
                    else:
                        cible,poscible,dict_cible=proche(e4[0],e4[1],e2,e3,de2,de3,per_4,Terrain_final)
                        deplacement(e4[0],e4[1],per_4,cible,Terrain_final,dict_esp4f,-4)
              elif len(proche(e4[0],e4[1],3,-3,dict_esp3m, dict_esp3f,per_4,Terrain_final))==3:
                  cible,poscible,dict_cible=proche(e4[0],e4[1],3,-3,dict_esp3m,dict_esp3f,per_4,Terrain_final)
                  if len(proche(e4[0],e4[1],3,-3,dict_esp3m, dict_esp3f,1,Terrain_final))==3:
                      predation(dict_esp4f,dict_cible,cible,e4[0],e4[1],poscible[0],poscible[1],Terrain_final)
...
i = Label(window, text="périmètre 4")
...
space.grid(column = 0, row = 28 )
space = Label(window, text = "                       ")
space.grid(column = 0, row = 30 )
...
c = [A, B, C, D, E, F, G, H, I, K, L, M, N, O, P, Q, R, S, T, U, V, W, X, Y, Z]
...
def clicked():
    ...
    global amr_e3
    global amr_e2
    global faim_perdue_e4
    global faim_perdue_e3
    global faim_perdue_e2
    global gestation_e4
    global gestation_e3
    global gestation_e2
    global len_e4
    ...
    Ligne = Dimension
    Colonne = Dimension
    environnement=float(B1)
    List_number_species=list(C1)
    age_max_esp2=int(D1)
    age_max_esp3=int(E1)
    age_max_esp4=int(F1)
    per_2 =int(G1)      #périmètre
    per_3=int(H1)
    per_4=int(I1)
    faim_e4 = int(K1)        # à partir de combien ils ont faim
    faim_e3 =int(L1)
    faim_e2 =int(M1)
    amr_e4=float(N1)    #age de maturité sexuelle
    amr_e3=float(O1)
    amr_e2=float(P1)
    faim_perdue_e4=float(Q1)
    faim_perdue_e3=float(R1)
    faim_perdue_e2=float(S1)
    gestation_e4=float(T1)
    gestation_e3=float(U1)
    gestation_e2=float(V1)
    len_e4=list(W1)
    len_e3=list(X1)
    len_e2=list(Y1)
    len_plant=list(Z1)
...
btn = Button(window, text="Lancer", command=clicked)
btn.grid(column=2, row=29)
info = Label(window, text = "Pour accéder a 'Lotka-Volterra' fermer cette fenetre ")
info.grid(column = 0, row = 32 )
