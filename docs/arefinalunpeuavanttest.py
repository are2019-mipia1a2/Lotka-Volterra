...
def creatdict_esp3(Terrain):    #dictionnaire de l'espece 3 uniquement
    ...
    for i in range(len(Terrain)) :
        for j in range(len(Terrain[i])) :
            if Terrain[i][j]==3 :
                desp3[i,j]=[1,80,0]       #1er elmt : âge 2eme elmt : jauge de faim
    return desp3
...
def creatdict_esp4(terrain):    #dictionnaire de l'espece 3 uniquement
    ...
    for i in range(len(terrain)) :
        for j in range(len(terrain[i])) :
            if terrain[i][j]==4 :
                desp4[i,j]=[1,80,0]        #1er elmt : âge 2eme elmt : jauge de faim
    return desp4
...
def predation_plant(d_pred,i,j,pos_cible_i,pos_cible_j):
    if dict_plante[pos_cible_i,pos_cible_j][0]-30 >0 :        
        d_pred[i,j][1]+=30
        dict_plante[pos_cible_i,pos_cible_j][0]-=30
    else :
    ...
    for _ in range(n):
        for e4 in list(dict_esp4) :
            if dict_esp4[e4][0]>=age_max_esp4 :           #Si l'animal est trop vieux
                ...
                Terrain_final[e4].remove(4)
                continue
            if dict_esp4[e4][1]<=15 :        #Si l'animal a faim
              dict_esp4[e4][1]-=5  
              dict_esp4[e4][0]+=1/12
              if dict_esp4[e4][2]!=0 :
                   dict_esp4[e4][2]-=1
            ...
            elif dict_esp4[e4][0]>=2 and dict_esp4[e4][2]==0:         #Si l'animal est suffisamment agé pour se reproduire
                dict_esp4[e4][1]-=5
                dict_esp4[e4][0]+=1/12
        ...
        for e3 in list(dict_esp3) :
            ...
            elif dict_esp3[e3][0]>=2 and dict_esp3[e3][2]==0:         #Si l'animal est suffisamment agé pour se reproduire
                dict_esp3[e3][1]+=-5
                dict_esp3[e3][0]+=1/12
        ...
        for e2 in list(dict_esp2) :
            ...
            elif dict_esp2[e2][0]>=0.5 and dict_esp2[e2][2]==0:
                dict_esp2[e2][1]+=-5
                dict_esp2[e2][0]+=1/12
...
def update(*args):          # fonction update appelee de maniere sequentielle
    state = np.random.random((10,10)) # on remplit la matrice de valeurs random
    im.set_array(state)     #
    return im,              # attention a la syntaxe im,
...
delay = 500  # en ms, le délai (minimum) entre chaque image

im = plt.imshow(state, interpolation='nearest')
ani = animation.FuncAnimation(Terrain_final, affichage, interval=delay, blit=False)

plt.show()
#youpi=animation.FuncAnimation(affichage,final,frames=None,init_func=None,interval=200)