...
def deplacement(i,j,per, cible, Terrain,dict_esp,esp):
    ...
    if len(k)>0:
        ...
        dict_esp[i+dx,j+dy]=dict_esp.pop((i,j))
        Terrain[i,j].remove(esp)
        Terrain[i+dx,j+dy].append(esp)
...
def perimetre_vision(per,i,j, cible,Terrain) : #s'applique sur un animal de position (i,j) qui cherche une proie de valeur cible
    ...
    for ligne in range(i-per+npsg,i+per-npsd-1):
        for colonne in range(j-per+npsh, j+per-npsb+1):
            ...
            if max(Terrain[ligne-1][colonne-1])==cible :
                if len(temp)==0 :
                    temp=[ligne, colonne]
...
def temp() :
    for e4 in list(dict_esp4) :
        print(e4[0], e4[1])
        print(len(perimetre_vision(1,e4[0],e4[1],4,Terrain_final)))
        perimetre_vision(1,e4[0],e4[1],4,Terrain_final)[0]
        perimetre_vision(1,e4[0],e4[1],4,Terrain_final)[1]
    ...
    for _ in range(n):
        for e4 in list(dict_esp4) :
            if dict_esp4[e4][0]>=age_max_esp4 :           #Si l'animal est trop vieux
                ...
                del dict_esp4[e4]
                Terrain_final[e4].remove(4)
            elif dict_esp4[e4][1]<=20 :        #Si l'animal a faim
                if len(perimetre_vision(1,e4[0],e4[1],3,Terrain_final))>0 and math.sqrt((perimetre_vision(1,e4[0],e4[1],3,Terrain_final)[0]-e4[0])**2+(perimetre_vision(1,e4[0],e4[1],3,Terrain_final)[1]-e4[1])**2)<=math.sqrt(2) :
                    predation(dict_esp4, dict_esp3, 3, e4[0],e4[1],perimetre_vision(1,e4,3,Terrain_final)[0] )
                else :
                ...
                if len(perimetre_vision(1,e4[0],e4[1],4,Terrain_final))>0 and math.sqrt((perimetre_vision(1,e4[0],e4[1],4,Terrain_final)[0]-e4[0])**2+(perimetre_vision(1,e4[0],e4[1],4,Terrain_final)[1]-e4[1])**2)<=math.sqrt(2) :
        ...
        for e3 in list(dict_esp3) :
            if dict_esp3[e3][0]>=age_max_esp3 :           #Si l'animal est trop vieux
            ...
            elif dict_esp3[e3][1] == 0 :         #Si l'animal meurt de faim
                del dict_esp3[e3]
                Terrain_final[e3].remove(3)
            elif dict_esp3[e3][1]<=20 :        #Si l'animal a faim
                if len(perimetre_vision(1,e4[0],e4[1],2,Terrain_final))>0 and math.sqrt((perimetre_vision(1,e3[0],e3[1],2,Terrain_final)[0]-e3[0])**2+(perimetre_vision(1,e3[0],e3[1],2,Terrain_final)[1]-e3[1])**2)<=math.sqrt(2) :
                ...
                if len(perimetre_vision(1,e4[0],e4[1],3,Terrain_final))>0 and math.sqrt((perimetre_vision(1,e3[0],e3[1],3,Terrain_final)[0]-e3[0])**2+(perimetre_vision(1,e3[0],e3[1],3,Terrain_final)[1]-e3[1])**2)<=math.sqrt(2) :
        ...
        for e2 in list(dict_esp2) :
            ...
            elif dict_esp2[e2][1] == 0 :         #Si l'animal est affamé
                del dict_esp2[e2]
                Terrain_final[e2].remove(2)
            elif dict_esp2[e2][1]<=20 :
                if len(perimetre_vision(1,e4[0],e4[1],1,Terrain_final))>0 and math.sqrt((perimetre_vision(1,e2[0],e2[1],1,Terrain_final)[0]-e2[0])**2+(perimetre_vision(1,e2[0],e2[1],1,Terrain_final)[1]-e2[1])**2)<=math.sqrt(2) :
                    predation_plant(dict_esp2,e2[0],e2[1],perimetre_vision(1,e2[0],perimetre_vision(1,e2[1])))
                else :
                    deplacement(e2[0],e2[1],per_2, 1, Terrain_final,dict_esp2,2)
            elif dict_esp2[e2][0]>=2 :
                if len(perimetre_vision(1,e4[0],e4[1],3,Terrain_final))>2 and math.sqrt((perimetre_vision(1,e2[0],e2[1],2,Terrain_final)[0]-e2[0])**2+(perimetre_vision(1,e2[0],e2[1],2,Terrain_final)[1]-e2[1])**2)<=math.sqrt(2) :
                    ...
                    deplacement(e2,per_2, 1, Terrain_final,dict_esp2,2)
           # else :
