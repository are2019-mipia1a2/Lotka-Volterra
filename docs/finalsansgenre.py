...
per_3=5
per_4=8
faim_e4 = 20        # à partir de combien ils ont faim
faim_e3 =25
faim_e2 =30
    ...
    if i==pos_cible_i :
        if j>pos_cible_j :
            if i!=len(terrain)-1 :
                ...
                if len(terrain[i,pos_cible_j-1])==1 :
                    d_pred[i,pos_cible_j-1]=[0,100,0]
        ...
        if i>pos_cible_i :
            if j!=len(terrain[0])-1 :
                if len(terrain[i,j+1])==0 :
                    d_pred[i,j+1]=[0,100,0]
                    terrain[i,j+1].append(espece)
                elif len(terrain[pos_cible_i,j+1])==0 :
                ...
                if len(terrain[i,j-1])==0 :
                    ...
                    terrain[pos_cible_i,j-1].append(espece)
            if i!=len(terrain)-1:
                if len(terrain[i+1,j])==0 :
                    d_pred[i+1,j]=[0,100,0]
                    terrain[i+1,j].append(espece)
            elif pos_cible_i!=0 :
                if len(terrain[pos_cible_i,j])==1 :
                    d_pred[pos_cible_i,j]=[0,100,0]
...
def cgmt(*args) :
    for e4 in list(dict_esp4) :
            ...
            if dict_esp4[e4][1]<=faim_e4 :        #Si l'animal a faim
              ...
              if len(dict_esp3)!=0 :                #Il mange l'espèce 3 en priorité sauf que s'il n'y en a plus alors il mange de l'espece 2
                  ...
                  if perimetre_vision(1,e4[0],e4[1],2,Terrain_final)[0]!=-1 :
                     #print(e4,"4","Mange")
                     predation(dict_esp4, dict_esp2, 2, e4[0],e4[1],perimetre_vision(1,e4[0],e4[1],2,Terrain_final)[0],perimetre_vision(1,e4[0],e4[1],2,Terrain_final)[1],Terrain_final)
                  elif perimetre_vision(per_4,e4[0],e4[1],2,Terrain_final)[0]!=-1 :
                           deplacement(e4[0],e4[1],per_4, 2, Terrain_final,dict_esp4,4)
    ...
    for e2 in list(dict_esp2) :
            ...
            if dict_esp2[e2][1] == 0 :         #Si l'animal est affamé
                ...
                if perimetre_vision(1,e2[0],e2[1],2,Terrain_final)[0]!=-1 :#and math.sqrt((perimetre_vision(1,e2[0],e2[1],2,Terrain_final)[0]-e2[0])**2+(perimetre_vision(1,e2[0],e2[1],2,Terrain_final)[1]-e2[1])**2)<=math.sqrt(2) :
                    ...
                    deplacement(e2[0],e2[1],per_2, 2, Terrain_final,dict_esp2,2)
            else :
               dict_esp2[e2][0]+=1/12
...
#Bug predation key error! -> Erreur déplacement (indice négatif)